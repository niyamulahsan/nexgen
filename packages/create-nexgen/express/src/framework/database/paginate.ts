...
function resolvePath(req: RequestLike, path?: string) {
  if (path) return path;
  const fallbackPath = req.path;
  if (!req.url) return fallbackPath;
  try {
...
export async function paginate<T = any>(
  ...
  const resolvedPath = resolvePath(req, options.path);
...
export async function paginateModel<T = any>(req: RequestLike, options: PaginateModelOptions<T>): Promise<PaginatedResult<T>> {
  const page = Number(req.query.page || options.page || 1);
  const size = req.query.size;
  const perPageQuery = req.query.per_page;
