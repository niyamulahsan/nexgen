...
/** Check if Yarn PnP is in use. */
function isYarnPnP() {
  try {
    const rcPath = path.resolve(process.cwd(), ".yarnrc.yml");
    if (fsSync.existsSync(rcPath)) {
      const rc = fsSync.readFileSync(rcPath, "utf-8");
      return !rc.includes("nodeLinker: node-modules");
    }
    const yarnPath = path.resolve(process.cwd(), ".yarn", "releases");
    return fsSync.existsSync(yarnPath);
  } catch {
    return false;
  }
}

/** Resolve a binary path via `yarn bin` (for Yarn PnP). */
function yarnBin(name) {
  try {
    const binPath = execSync(`yarn bin ${name}`, { cwd: process.cwd(), encoding: "utf-8" }).trim();
    if (binPath && fsSync.existsSync(binPath)) return binPath;
  } catch {}
  return null;
}
